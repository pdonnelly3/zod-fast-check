...
import { INVALID, OK, ParseInput, ParseReturnType, ZodSchema, ZodTypeAny, ZodTypeDef } from 'zod';
import { ZodFastCheck, ZodFastCheckGenerationError, ZodFastCheckUnsupportedSchemaError } from './zod-fast-check-module-proxy';
...
describe('Generate arbitraries for Zod schema input types', () => {
	...
	const schemas: Record<string, () => ZodSchema> = {
		...
		'map with object keys': () => z.map(z.object({ id: z.number() }), z.array(z.boolean())),
		set: () => z.set(z.number()),
...
describe('Generate arbitraries for Zod schema output types', () => {
	...
	test('transformer within a transformer', () => {
		...
		const schema = z.array(z.boolean().transform((bool) => `${bool}!`)).transform((array) => array.join(''));
...
describe('Override the arbitrary for a particular schema type', () => {
	...
	test('using custom integer arbitrary for IntAsString input', () => {
		const arbitrary = ZodFastCheck().override(IntAsString, fc.integer()).inputOf(IntAsString);
	...
	test('using custom integer arbitrary for IntAsString output', () => {
		const arbitrary = ZodFastCheck().override(IntAsString, fc.integer()).outputOf(IntAsString);
	...
	test('using a function to lazily define an override', () => {
		...
		const zfc = ZodFastCheck().override(NumericString, (zfc) => zfc.inputOf(z.number().int().nonnegative()).map(String));
...
describe('Throwing an error if the schema type is not supported', () => {
	test('lazy schemas', () => {
		expect(() => ZodFastCheck().inputOf(z.lazy(() => z.string()))).toThrow(
			new ZodFastCheckUnsupportedSchemaError(
				'Unable to generate valid values for Zod schema. ' + "Lazy schemas are not supported (at path '.')."
			)
		);
	});
	...
	test('never schemas', () => {
		expect(() => ZodFastCheck().inputOf(z.never())).toThrow(
			new ZodFastCheckUnsupportedSchemaError(
				'Unable to generate valid values for Zod schema. ' + "Never schemas are not supported (at path '.')."
			)
		);
	});

	test('intersection schemas', () => {
		expect(() => ZodFastCheck().inputOf(z.intersection(z.object({ foo: z.string() }), z.object({ bar: z.number() })))).toThrow(
			new ZodFastCheckUnsupportedSchemaError(
				'Unable to generate valid values for Zod schema. ' + "Intersection schemas are not supported (at path '.')."
			)
		);
	});
	...
	test('third-party schemas', () => {
		...
		expect(() => ZodFastCheck().inputOf(new SymbolSchema({ symbol: Symbol.iterator }))).toThrow(
			new ZodFastCheckUnsupportedSchemaError(
				'Unable to generate valid values for Zod schema. ' + "'SymbolSchema' schemas are not supported (at path '.')."
			)
		);
	});
});

function testIfSchemaSupported(name: string, buildSchema: () => ZodSchema, testBody: (s: ZodSchema) => void | Promise<void>): void {
