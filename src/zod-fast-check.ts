...
const ZOD_EMAIL_REGEX = /^(?!\.)(?!.*\.\.)([A-Z0-9_'+\-\.]*)[A-Z0-9_+-]@([A-Z0-9][A-Z0-9\-]*\.)+[A-Z]{2,}$/i;

type UnknownZodSchema = ZodSchema<unknown, ZodTypeDef, unknown>;

type SchemaToArbitrary = <Schema extends UnknownZodSchema>(schema: Schema, path: string) => Arbitrary<Schema['_input']>;

type ArbitraryBuilder<Schema extends UnknownZodSchema> = (schema: Schema, path: string, recurse: SchemaToArbitrary) => Arbitrary<Schema['_input']>;

type ArbitraryBuilders = {
	[TypeName in ZodFirstPartyTypeKind]: ArbitraryBuilder<ExtractFirstPartySchemaType<TypeName>>;
};
...
type ExtractFirstPartySchemaType<TypeName extends ZodFirstPartyTypeKind> = Extract<AllFirstPartySchemaTypes, { _def: { typeName: TypeName } }>;
...
type OverrideArbitrary<Input = unknown> = Arbitrary<Input> | ((zfc: ZodFastCheck) => Arbitrary<Input>);

class _ZodFastCheck {
	private overrides = new Map<ZodSchema<unknown, ZodTypeDef, unknown>, OverrideArbitrary>();
	...
	private inputWithPath<Input>(schema: ZodSchema<unknown, ZodTypeDef, Input>, path: string): Arbitrary<Input> {
		...
		if (isFirstPartyType(schema)) {
			const builder = arbitraryBuilders[schema._def.typeName] as ArbitraryBuilder<typeof schema>;
	...
	outputOf<Schema extends UnknownZodSchema>(schema: Schema): Arbitrary<output<Schema>> {
		...
		if (isFirstPartyType(schema) && SCALAR_TYPES.has(`${(schema as AllFirstPartySchemaTypes)._def.typeName}` as const)) {
	...
	private findOverride<Input>(schema: ZodSchema<unknown, ZodTypeDef, Input>): Arbitrary<Input> | null {
		const override = this.overrides.get(schema);

		if (override) {
			return (typeof override === 'function' ? override(this) : override) as Arbitrary<Input>;
		}
...
function isFirstPartyType(schema: UnknownZodSchema): schema is AllFirstPartySchemaTypes {
	const typeName = (schema._def as { typeName?: string }).typeName;
	return !!typeName && Object.prototype.hasOwnProperty.call(arbitraryBuilders, typeName);
}
...
const arbitraryBuilders: ArbitraryBuilders = {
	...
	ZodArray(schema: ZodArray<UnknownZodSchema>, path: string, recurse: SchemaToArbitrary) {
	...
	ZodObject(schema: ZodObject<ZodRawShape>, path: string, recurse: SchemaToArbitrary) {
		const propertyArbitraries = objectFromEntries(
			Object.entries(schema._def.shape()).map(([property, propSchema]) => [property, recurse(propSchema, path + '.' + property)])
		);
		return fc.record(propertyArbitraries);
	},
	ZodUnion(schema: ZodUnion<[UnknownZodSchema, ...UnknownZodSchema[]]>, path: string, recurse: SchemaToArbitrary) {
		return fc.oneof(...schema._def.options.map((option) => recurse(option, path)));
	},
	ZodIntersection(_, path: string) {
		unsupported(`Intersection`, path);
	},
	ZodTuple(schema: ZodTuple, path: string, recurse: SchemaToArbitrary) {
		return fc.tuple(...schema._def.items.map((item, index) => recurse(item, `${path}[${index}]`)));
	},
	ZodRecord(schema: ZodRecord, path: string, recurse: SchemaToArbitrary) {
		return fc.dictionary(recurse(schema._def.keyType, path), recurse(schema._def.valueType, path + '[*]'));
	},
	...
	ZodFunction(schema: ZodFunction<ZodTuple, UnknownZodSchema>, path: string, recurse: SchemaToArbitrary) {
	...
	ZodPromise(schema: ZodPromise<UnknownZodSchema>, path: string, recurse: SchemaToArbitrary) {
	...
	ZodOptional(schema: ZodOptional<UnknownZodSchema>, path: string, recurse: SchemaToArbitrary) {
		const nil = undefined;
	...
	ZodNullable(schema: ZodNullable<UnknownZodSchema>, path: string, recurse: SchemaToArbitrary) {
		const nil = null;
		return fc.option(recurse(schema._def.innerType, path), {
			nil,
			freq: 2
	...
	ZodDefault(schema: ZodDefault<UnknownZodSchema>, path: string, recurse: SchemaToArbitrary) {
		return fc.oneof(fc.constant(undefined), recurse(schema._def.innerType, path));
	},
	ZodEffects(schema: ZodEffects<UnknownZodSchema>, path: string, recurse: SchemaToArbitrary) {
		const preEffectsArbitrary = recurse(schema._def.schema, path);

		return filterArbitraryBySchema(preEffectsArbitrary, schema, path);
	},
	ZodDiscriminatedUnion(schema: ZodDiscriminatedUnion<string, ZodDiscriminatedUnionOption<string>[]>, path: string, recurse: SchemaToArbitrary) {
	...
	ZodCatch(schema: ZodCatch<UnknownZodSchema>, path: string, recurse: SchemaToArbitrary) {
		return fc.oneof(recurse(schema._def.innerType, path), fc.anything());
	},
	ZodPipeline(schema: ZodPipeline<UnknownZodSchema, UnknownZodSchema>, path: string, recurse: SchemaToArbitrary) {
		return recurse(schema._def.in, path).filter(
			throwIfSuccessRateBelow(MIN_SUCCESS_RATE, (value): value is typeof value => schema.safeParse(value).success, path)
		);
	},
	ZodSymbol() {
...
function createDatetimeStringArb(schema: ZodString, check: { precision: number | null; offset: boolean }): Arbitrary<string> {
	let arb = fc
		.date({
...
const isUnionMember =
	...
	(value: T): value is Extract<T, Filter> => {
		return Object.entries(filter).every(([key, expected]) => value[key as keyof T] === expected);
	};

function filterArbitraryBySchema<T>(arbitrary: Arbitrary<T>, schema: ZodSchema<unknown, ZodTypeDef, T>, path: string): Arbitrary<T> {
	return arbitrary.filter(throwIfSuccessRateBelow(MIN_SUCCESS_RATE, (value): value is typeof value => schema.safeParse(value).success, path));
}
	...
	return (value: Value): value is Refined => {
		...
		if (total > MIN_RUNS && successful / total < rate) {
			throw new ZodFastCheckGenerationError(
				'Unable to generate valid values for Zod schema. ' + `An override is must be provided for the schema at path '${path || '.'}'.`
			);
		}
...
function objectFromEntries<Value>(entries: Array<[string, Value]>): Record<string, Value> {
...
const getValidEnumValues = (obj: Record<string | number, string | number>): unknown[] => {
